export const bio = {
  tagline: "Hardware engineer who builds plasma reactors and underwater robots by day, ships AI agents by night.",
  full: "Founded 3 ventures, published 3 peer-reviewed papers. All before graduating. Currently running PlasmaX, building an open-source ROV, and shipping an AI-powered insurance auditor. Lavin Entrepreneurship Fellow at UW.",
  systemInfo: {
    os: "TejasOS v1.0",
    build: "Founder Edition",
    user: "Tejas Naladala",
    location: "Seattle, WA",
    processor: "UW ECE + Applied Math (Double Major, 3.93 GPA)",
    status: "Lavin Entrepreneurship Fellow",
    uptime: "Since May 2023",
    activeProcesses: [
      "PlasmaX (Founder & CEO)",
      "Cerulean Robotics (Founder)",
      "Atticus AI (Founder & Technical Lead)",
      "UW Class of 2028",
    ],
    languages: "English, Hindi, Telugu | Beginner Mandarin",
  },
};
